...
class Place(object):
	...
	def remove_insect(self, insect):
		...
		"""
		if (isinstance(insect, QueenAnt) and insect.impostor) or not isinstance(insect, QueenAnt):
			if insect.is_ant:
...
# END Problem 13
	...
	def action(self, colony):
		...
		#act as a queen if not imposter
		else:
			ScubaThrower.action(self, colony)
			pre_place = self.place.exit
			while pre_place != None:
				if pre_place.ant:
					if pre_place.ant.damage == type(pre_place.ant).damage:
						pre_place.ant.damage *= 2
					if pre_place.ant.container and pre_place.ant.ant != None:
						if pre_place.ant.ant.damage == type(pre_place.ant.ant).damage:
							pre_place.ant.ant.damage *= 2
				pre_place = pre_place.exit
		# END Problem 13
...
def apply_effect(effect, bee, duration):
	...
	# BEGIN Problem EC
	pre_action = bee.action
	remain_turns = duration
	def curr_action(colony):
		nonlocal remain_turns
		if remain_turns > 0:
			remain_turns -= 1
			effect(pre_action)(colony)
		else:
			pre_action(colony)
